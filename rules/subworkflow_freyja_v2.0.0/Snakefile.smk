...
# Run pipeline solo:
# snakemake --snakefile rules/subworkflow_freyja_v2.0.0/Snakefile.smk --use-conda -c1

rule _all:
    input:
        expand("experiments/{exp}/results/postPrediction/freyja_v2.0.0_summary.csv", 
...
rule _generateInput: # generate depth file, unzip vcf, variants and demix
    ...
    output:
        depth="experiments/{exp}/results/freyja_v2.0.0/{sample}.depth"
    conda: 
        f"../../envs/freyja_v2.0.0.yaml"
    log:
        "logs/{exp}/freyja_v2.0.0/{sample}.input.out"
    shell:  
...
rule _main: # start main freyja step
    input:
        ...
        ref=merged_config["REFERENCES"]["REF_GENOME"]
    output:
        tsv="experiments/{exp}/results/freyja_v2.0.0/{sample}.tsv"
    conda: 
        f"../../envs/freyja_v2.0.0.yaml"
    log:
        "logs/{exp}/freyja_v2.0.0/{sample}.demix.out"
    shell: 
        "gunzip -kf {input.vcf} && "
...
checkpoint _prepareSummary:
    input:
        expand("experiments/{exp}/results/freyja_v2.0.0/{sample}.tsv", 
        exp=merged_config["EXPERIMENT_NAME"], 
        sample=merged_config["SAMPLES"])
    output:
        "experiments/{exp}/results/postPrediction/freyja_v2.0.0_summary.csv"
    conda:
        "../../envs/freyja_v2.0.0.yaml"
    params:
        min_threshold = merged_config["POSTPRED"]["LINEAGE_MIN_THRESHOLD"]
    log:
        "logs/{exp}/freyja_v2.0.0/PostPred_summary.log"
    shell:
        "python rules/subworkflow_freyja_v2.0.0/prepareSummary.py -i {input} "
        "-n freyja_v2.0.0 -m {params.min_threshold} -o {output}"