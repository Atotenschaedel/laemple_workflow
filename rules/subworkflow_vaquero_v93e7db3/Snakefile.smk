...
TOOL_NAME="vaquero_v93e7db3"

# Run pipeline solo:
# snakemake --snakefile rules/subworkflow_vaquero_v93e7db3/Snakefile.smk --use-conda -c1

rule _all:
    input:
        expand("experiments/{exp}/results/postPrediction/vaquero_v93e7db3_summary.csv", 
...
rule _generateInputAF: #generate tidy table from vcf files 
    ...
    output:
        temp("experiments/{exp}/results/vaquero_v93e7db3/{sample}_af.tsv")
    params: 
        sample_suffix = merged_config["SAMPLE_SUFFIX"]
    conda: 
        "../../envs/pysam.yaml"
    shell:
        "python3 bin/vaquero_v93e7db3/scripts/vcf2tsv_long.py -i {input} -m 0.1 | "
...
rule _mergeAFs: # merge tidy tables togehter, remove all but first header
    input: 
        expand("experiments/{exp}/results/vaquero_v93e7db3/{sample}_af.tsv", 
        exp=merged_config["EXPERIMENT_NAME"], sample=merged_config["SAMPLES"])
    output: 
        expand("experiments/{exp}/results/vaquero_v93e7db3/Input_afs.tsv", 
        exp=merged_config["EXPERIMENT_NAME"])
    shell: 
...
rule _generateInputMetaData: # generate metadata for vaquero based on generic metadata file
    ...
    output: 
        "experiments/{exp}/results/vaquero_v93e7db3/Input_metadata.tsv"
    conda: 
        "../../envs/python3.yaml"
    params: 
        ...
        sample_suffix=merged_config["SAMPLE_SUFFIX"]
    shell: 
        "python3 rules/subworkflow_vaquero_v93e7db3/generateMetadata.py --input {input} --output {output} "
...
rule _main: # start main vaquero step
    input:
        afs="experiments/{exp}/results/vaquero_v93e7db3/Input_afs.tsv",
        meta="experiments/{exp}/results/vaquero_v93e7db3/Input_metadata.tsv"
    output: 
        "experiments/{exp}/results/vaquero_v93e7db3/globalFittedData.csv"
    params:
        markerDate = merged_config["MARKER_DATE"],
        markerLocation = merged_config["MARKER_LOCATION"]
    conda: 
        "../../envs/vaquero_v93e7db3.yaml"
    log: 
        "logs/{exp}/vaquero_v93e7db3/vaquero.out"
    shell:
        "Rscript bin/custom_scripts/install_devtools.R &>{log} && "
        "Rscript bin/vaquero_v93e7db3/scripts/VaQuERo_v2.r "
        "--dir experiments/{wildcards.exp}/results/vaquero_v93e7db3/ "
        "--metadata {input.meta} --data {input.afs} "
        "--marker bin/vaquero_v93e7db3/resources/mutations_list_grouped_pango_codonPhased_{params.markerDate}_{params.markerLocation}.csv "
        "--smarker bin/vaquero_v93e7db3/resources/mutations_special_2022-12-21.csv "
        "--pmarker bin/vaquero_v93e7db3/resources/mutations_problematic_2024-10-04.csv "
        "--voi=XBB.1.16,XBB.2.3,FE.1 --colorBase=XBB,BA.1,BA.2,BA.5 >>{log} 2>&1" 

checkpoint _prepareSummary:
    input:
        fitted="experiments/{exp}/results/vaquero_v93e7db3/globalFittedData.csv",
        meta="experiments/{exp}/simulation/{exp}_metadata.tsv"
    output:
        "experiments/{exp}/results/postPrediction/vaquero_v93e7db3_summary.csv"
    conda:
        "../../envs/python3.yaml"
    params:
        min_threshold = merged_config["POSTPRED"]["LINEAGE_MIN_THRESHOLD"]
    log:
        "logs/{exp}/vaquero_v93e7db3/PostPred_summary.csv"
    shell:
        "python rules/subworkflow_vaquero_v93e7db3/prepareSummary.py -i {input.fitted} "
        "--metafile {input.meta} --name vaquero_v93e7db3 "
