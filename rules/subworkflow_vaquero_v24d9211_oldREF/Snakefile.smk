...
TOOL_NAME="vaquero_v24d9211_oldREF"

# Run pipeline solo:
# snakemake --snakefile rules/subworkflow_vaquero_v24d9211_oldREF/Snakefile.smk --use-conda -c1

rule _all:
    input:
        expand("experiments/{exp}/results/postPrediction/vaquero_v24d9211_oldREF_summary.csv", 
...
rule _generateInputAF: #generate tidy table from vcf files 
    ...
    output:
        temp("experiments/{exp}/results/vaquero_v24d9211_oldREF/{sample}_af.tsv")
    params: 
...
rule _mergeAFs: # merge tidy tables togehter, remove all but first header
    input: 
        expand("experiments/{exp}/results/vaquero_v24d9211_oldREF/{sample}_af.tsv", 
        exp=merged_config["EXPERIMENT_NAME"], sample=merged_config["SAMPLES"])
    output: 
        expand("experiments/{exp}/results/vaquero_v24d9211_oldREF/Input_afs.tsv", 
        exp=merged_config["EXPERIMENT_NAME"])
    shell: 
...
rule _generateInputMetaData: # generate metadata for vaquero based on generic metadata file
    ...
    output: 
        "experiments/{exp}/results/vaquero_v24d9211_oldREF/Input_metadata.tsv"
    conda: 
        "../../envs/python3.yaml"
    params: 
        ...
        sample_suffix=merged_config["SAMPLE_SUFFIX"]
    shell: 
        "python3 rules/subworkflow_vaquero_v24d9211_oldREF/generateMetadata.py --input {input} --output {output} "
...
rule _main: # start main vaquero step
    input:
        afs="experiments/{exp}/results/vaquero_v24d9211_oldREF/Input_afs.tsv",
        meta="experiments/{exp}/results/vaquero_v24d9211_oldREF/Input_metadata.tsv"
    output: 
        "experiments/{exp}/results/vaquero_v24d9211_oldREF/globalFittedData.csv"
    params:
    ...
    log: 
        "logs/{exp}/vaquero_v24d9211_oldREF/vaquero.out"
    shell:
        ...
        "--dir experiments/{wildcards.exp}/results/vaquero_v24d9211_oldREF/ >>{log} 2>&1" 

checkpoint _prepareSummary:
    input:
        fitted="experiments/{exp}/results/vaquero_v24d9211_oldREF/globalFittedData.csv",
        meta="experiments/{exp}/simulation/{exp}_metadata.tsv"
    output:
        "experiments/{exp}/results/postPrediction/vaquero_v24d9211_oldREF_summary.csv"
    conda:
        "../../envs/python3.yaml"
    params:
        min_threshold = merged_config["POSTPRED"]["LINEAGE_MIN_THRESHOLD"]
    log:
        "logs/{exp}/vaquero_v24d9211_oldREF/PostPred_summary.csv"
    shell:
        "python rules/subworkflow_vaquero_v24d9211_oldREF/prepareSummary.py -i {input.fitted} "
        "--metafile {input.meta} --name vaquero_v24d9211_oldREF "
