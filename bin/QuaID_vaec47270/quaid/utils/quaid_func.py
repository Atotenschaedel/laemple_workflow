...
def get_all_voc(level):
    ...
    if level == 4:
        ...
        WHO_variants['BA.1'].append("B.1.1.529.1")
        WHO_variants['BA.2+BA.3'].append("B.1.1.529.2")
        WHO_variants['BA.4'].append("B.1.1.529.4")
        WHO_variants['BA.5'].append("B.1.1.529.5")
...
def get_quasiunique_df(recent_nt_df, WHO_variants, alias_data, occurence_cutoff, incl_cutoff, excl_cutoff, level=4, 
                       save_to_disk=None, verbose=True, date='0'):
    if level == -2:
        level = 5
    ...
    if not save_to_disk is None:
        recent_nt_df.to_csv(f'{save_to_disk}/QU-{date}.csv')
...
def get_result_table(vcf_df, quasiunique_df, coverage_data, WHO_variants, date, save=False, min_cov=0., verbose=True):
    vcf_data = vcf_df.merge(quasiunique_df, how='inner', on='NT_mut')
    if save:
        vcf_data.to_csv(f'output/{pd.to_datetime(date).strftime("%Y-%m-%d")}-vcf.csv')
    ...
    for variant in WHO_variants:
        ...
        for p in plants:
            ...
            if flag:
                for _, nt_mut in lin_mut_nt.iterrows():
                    cov = 0

                    try:
                        if nt_mut['NT_mut'][-1] == "-":
                            cov = int(cov_data[cov_data[1] == (int(nt_mut['NT_mut'][1:-1]) - 20)].iloc[0, 2]) + \
                                  int(cov_data[cov_data[1] == (int(nt_mut['NT_mut'][1:-1]) + 20)].iloc[0, 2])
                        else:
                            cov = int(cov_data[cov_data[1] == int(nt_mut['NT_mut'][1:-1])].iloc[0, 2])
