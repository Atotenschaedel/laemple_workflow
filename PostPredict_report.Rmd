...
knitr::kable(data.frame(Tools = names(tool_list)) %>% rowwise() %>% mutate(`Tool name` = unlist(str_split(Tools, "_"))[1], Version = unlist(str_split(Tools, "_"))[2], Reference = unlist(str_split(Tools, "_"))[3]) %>% dplyr::select(-Tools) %>% mutate(Reference = ifelse(is.na(Reference), "default", Reference)))
```
...
if( length(unique(data.tool.long$replicate)) > 1){
  for (timecourse_name in tabs){
...
```{r def-density-Plot-func, results='asis', fig.width=10, fig.height=5}
  ...
  for (tool in names(tool_list)){
    ...
    p <- annotate_figure(
      ggarrange(
        ...
        ), top = text_grob("Metrics comparison between replicates", face = "bold", size = 14))
...
```{r experiments-comparison-densityPlots, echo=FALSE, results='asis', fig.width=10, fig.height=5}
...
for (timecourse_name in tabs){
 ...
 p <- annotate_figure(
      ...
      top = text_grob(paste0("Metrics comparison between experiments -",timecourse_name), face = "bold", size = 14))
...
```{r def-lineage-identification-metrics-func}
...
```{r abundance-estimation-error-calc}
data.error <- NULL
...
```{r abundance-estimation-plot-loop, fig.height=8}
