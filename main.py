...
if config["INCLUDE_SIMULATION_PIPELINE"]:
    for dataset in config["SIMULATION"]["DATASET"]:
        ...
        # parameter modifications as experiment name, quality score, seed
        experiment_param = {}
        ...
        for index, experiment in enumerate(experiment_param.keys()):
            ...
            # create workflow_copy for log
            with open(f"logs/{experiment}/workflow_config_COPY.yaml", "w") as cfile:
                yaml.dump(config, cfile)
    ...
    # create workflow_copy for log
    with open("logs/"+config["EXPERIMENT_NAME"]+"/workflow_config_COPY.yaml", "w") as cfile:
        yaml.dump(config, cfile)

    cli = ""
    cli += f"snakemake --snakefile rules/variantCalling.smk -c{cores} --rerun-incomplete  --use-conda"
    cli += " && "
    cli += f"snakemake --snakefile rules/lineage_deconvolution.smk -c{cores} --use-conda --keep-going --rerun-incomplete --rerun-triggers mtime"
    cli += ""
